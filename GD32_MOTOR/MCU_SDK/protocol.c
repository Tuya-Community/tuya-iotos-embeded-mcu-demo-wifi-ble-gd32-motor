...
unsigned char fwd_flag=0;
unsigned char rev_flag=0;
unsigned char vlo_flag=0;
...
{
  {DPID_SBF, DP_TYPE_BOOL},
  {DPID_REV, DP_TYPE_BOOL},
  {DPID_VLO, DP_TYPE_BOOL},
  {DPID_STOP, DP_TYPE_BOOL},
  {DPID_PWM, DP_TYPE_VALUE},
};
...
void all_data_update(void)
{
   // #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
    ...
    mcu_dp_value_update(DPID_PWM,当前PWM值); //VALUE型数据上报;
...
******************************************************************************/
/*****************************************************************************
函数名称 : dp_download_sbf_handle
功能描述 : 针对DPID_SBF的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 只下发类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_sbf_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    sbf = mcu_get_dp_download_bool(value,length);
    if(sbf == 0) {
        //开关关	 
    fwd_flag=0;	
    }else {
    ...
    rev_flag=0;	
    vlo_flag=0;				
	  mcu_dp_bool_update(DPID_REV,rev_flag);
		mcu_dp_bool_update(DPID_VLO,vlo_flag);
    }
  
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_SBF,sbf);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_rev_handle
功能描述 : 针对DPID_REV的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 只下发类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_rev_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    rev = mcu_get_dp_download_bool(value,length);
    if(rev == 0) {
        //开关关
      rev_flag=0;	
    }else {
   ...
   vlo_flag=0;						
   fwd_flag=0;			
  mcu_dp_bool_update(DPID_SBF,fwd_flag);	
  mcu_dp_bool_update(DPID_VLO,vlo_flag);	
    }
  
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_REV,rev);
...
/*****************************************************************************
函数名称 : dp_download_vlo_handle
功能描述 : 针对DPID_VLO的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 只下发类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_vlo_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    vlo = mcu_get_dp_download_bool(value,length);
    if(vlo == 0) {
        //开关关
			 vlo_flag=0;			
    }else {
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_VLO,vlo);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_stop_handle
功能描述 : 针对DPID_STOP的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 只下发类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_stop_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    stop = mcu_get_dp_download_bool(value,length);
    if(stop == 0) {
        //开关关
			stop_flag=0;	
    }else {
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_STOP,stop);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_pwm_handle
功能描述 : 针对DPID_PWM的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_pwm_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long pwm;
    
    pwm = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    ...
    */
     pulse=pwm*10;
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_PWM,pwm);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        case DPID_SBF:
            //正转处理函数
            ret = dp_download_sbf_handle(value,length);
        break;
        case DPID_REV:
            //反转处理函数
            ret = dp_download_rev_handle(value,length);
        break;
        case DPID_VLO:
            //待机处理函数
            ret = dp_download_vlo_handle(value,length);
        break;
        case DPID_STOP:
            //停止处理函数
            ret = dp_download_stop_handle(value,length);
        break;
        case DPID_PWM:
            //PWM值处理函数
            ret = dp_download_pwm_handle(value,length);
        break;
